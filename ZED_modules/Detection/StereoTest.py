...
class StereoCamera():
    ...
    def run(self):
        ...
        while not self.exit_signal:
            if self.zed.grab(self.runtime_params) == sl.ERROR_CODE.SUCCESS:
                ...
                self.target_z_dist = self.stereo_measure()
    ...
    def stereo_measure(self):
        ...
        if len(self.YOLO_detections_left) == len(self.YOLO_detections_right) == 1:
            center_left = (self.YOLO_detections_left[0].bounding_box_2d[0][0] +
                           self.YOLO_detections_left[0].bounding_box_2d[1][0])/2
            
            center_right = (self.YOLO_detections_right[0].bounding_box_2d[0][0] +
                            self.YOLO_detections_right[0].bounding_box_2d[1][0])/2
            
            disparity = abs(center_left - center_right)
            self.target_z_dist = (self.focal_x_left * self.camera_baseline)/disparity
            
            print("Disparity is: " + str(disparity))
            print("Distance is: " + str(self.target_z_dist))
